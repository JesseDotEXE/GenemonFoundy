...
export class ActorFFG extends Actor {
  ...
  _prepareSharedData(actorData) {
    ...
    if (actorData.type === "minion" || actorData.type === "character") {
      ...
      if (game.settings.get("genemon", "enableSoakCalc")) {
  ...
  _prepareCharacterData(actorData) {
    ...
    if ((game.settings.get("genemon", "talentSorting") && (!actorData.flags?.config?.talentSorting || actorData.flags?.config?.talentSorting === "0")) || actorData.flags?.config?.talentSorting === "1") {
    ...
    actorData.modifiers[name] = Object.keys(properties).map((k) => {
      ...
      if (!attr) {
        ...
        if (key === "Defence-Melee") {
          ...
          try {
            if ((typeof actorData.flags?.genemon?.config?.enableAutoSoakCalculation === undefined && game.settings.get("genemon", "enableSoakCalc")) || actorData.flags?.genemon?.config?.enableAutoSoakCalculation) {
              value = 0;
            }
          } catch (err) {
