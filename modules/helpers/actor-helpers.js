      ...
      if (this.object.type !== "homestead") {
        if (this.object.type !== "vehicle") {
          ...
          Object.keys(CONFIG.FFG.character_stats).forEach((k) => {
            ...
            let y = parseInt(formData.data.attributes[key].value, 10) + x;
            if (key === "Soak") {
              const autoSoakCalculation = (typeof this.actor.flags?.genemon?.config?.enableAutoSoakCalculation === "undefined" && game.settings.get("genemon", "enableSoakCalc")) || this.actor.flags.genemon?.config.enableAutoSoakCalculation;
