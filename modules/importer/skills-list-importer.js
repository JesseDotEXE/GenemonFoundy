...
export default class SkillListImporter extends FormApplication {
  ...
  static get defaultOptions() {
    return mergeObject(super.defaultOptions, {
      ...
      width: 385,
      template: "systems/genemon/templates/importer/skill-list-importer.html",
    });
  }
  ...
  async getData() {
    ...
    const currentSkillTheme = await game.settings.get("genemon", "skilltheme");
  ...
  activateListeners(html) {
    ...
    html.find(".reset-button").on("click", async (event) => {
      ...
      game.settings.set("genemon", "arraySkillList", defaultSkillList);
      game.settings.set("genemon", "skilltheme", "starwars");
    ...
    html.find(".fa-download").on("click", async (event) => {
      ...
      const currentSkillList = await game.settings.get("genemon", "arraySkillList");
    ...
    html.find(".dialog-button").on("click", async (event) => {
      ...
      let currentSkillList = await game.settings.get("genemon", "arraySkillList");
      ...
      await game.settings.set("genemon", "arraySkillList", newMasterSkillListData);
      debounce(() => window.location.reload(), 100);
