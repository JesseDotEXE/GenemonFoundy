...
export class GroupManager extends FormApplication {
  ...
  static get defaultOptions() {
    return mergeObject(super.defaultOptions, {
      classes: ["genemon", "form", "group-manager"],
      closeOnSubmit: false,
      submitOnChange: true,
      submitOnClose: true,
      popOut: true,
      ...
      width: 330,
      height: 900,
      template: "systems/genemon/templates/group-manager.html",
      id: "group-manager",
      title: "Group Manager",
    });
  }
  ...
  getData() {
    ...
    const pcListMode = game.settings.get("genemon", "pcListMode");
    const characters = [];
    ...
    const dPool = { light: game.settings.get("genemon", "dPoolLight"), dark: game.settings.get("genemon", "dPoolDark") };
  ...
  activateListeners(html) {
    ...
    html.find(".initiative-mode").change((ev) => {
      ...
      game.settings.set("genemon", "initiativeRule", init_value);
  ...
  _updateObject(event, formData) {
    ...
    game.settings.set("genemon", "dPoolLight", formDPool.light);
    game.settings.set("genemon", "dPoolDark", formDPool.dark);
    return formData;
  }
  ...
  async _rollTable(table, type) {
    ...
    await r.evaluate();
    let rollOptions = game.settings.get("genemon", "privateTriggers") ? { rollMode: "gmroll" } : {};
    r.toMessage(
      {
    ...
    if (game.settings.get("genemon", "privateTriggers")) {
  ...
  async _grantXP(character) {
    ...
    const content = await renderTemplate("systems/genemon/templates/grant-xp.html", {
      id,
    });
