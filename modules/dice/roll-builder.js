export default class RollBuilderFFG extends FormApplication {
  ...
  static get defaultOptions() {
    return mergeObject(super.defaultOptions, {
      id: "roll-builder",
      classes: ["genemon", "roll-builder-dialog"],
      template: "systems/genemon/templates/dice/roll-options-ffg.html",
    });
  }
  ...
  async getData() {
    ...
    let canUserAddAudio = await game.settings.get("genemon", "allowUsersAddRollAudio");
    ...
    if (game.user.isGM) {
      game.playlists.contents.forEach((playlist) => {
        playlist.sounds.forEach((sound) => {
          let selected = false;
          const s = this.roll?.sound ?? this.roll?.item?.flags?.genemon?.ffgsound;
          if (s === sound.path) {
            selected = true;
          }
      ...
      const playlistId = await game.settings.get("genemon", "allowUsersAddRollAudioPlaylist");
    ...
    const enableForceDie = game.settings.get("genemon", "enableForceDie");
    const labels = {
      light: game.settings.get("genemon", "destiny-pool-light"),
      dark: game.settings.get("genemon", "destiny-pool-dark"),
    };
  ...
  activateListeners(html) {
    ...
    html.find(".btn").click(async (event) => {
      ...
      if (!this.roll.sound) {
        ...
        if (sound) {
          ...
          if (this?.roll?.item) {
            let entity;
            let entityData;
            if (!this?.roll?.item?.flags?.genemon?.uuid) {
              entity = game.actors.get(this.roll.data.actor._id);
              entityData = {
                _id: this.roll.item.id,
              };
            } else {
              const parts = this.roll.item.flags.genemon?.uuid.split(".");
            ...
            setProperty(entityData, "flags.genemon.ffgsound", sound);
      ...
      if (this.roll.hasOwnProperty('item') && this.roll.item !== undefined && this.roll.item.hasOwnProperty('flags') && !this.roll.item.flags.genemon.uuid) {
        // uuid is missing, look up the item and set it, so it's fixed going forward
        let tmp_item = await fromUuid(this.roll.item.uuid);
        await tmp_item.setFlag("genemon", "uuid", this.roll.item.uuid);
      }
      ...
      if (sentToPlayer) {
        ...
        if (this.roll.item.hasOwnProperty('crew') && Object.keys(this.roll.item).length > 1) {
          await this.roll.item.update({"flags": {"genemon": {"crew": this.roll.item.crew}}})
        }
