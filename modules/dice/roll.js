...
export class RollFFG extends Roll {
  ...
  async render(chatOptions = {}) {
    ...
    // Define chat data
    if (this?.data) {
      if (this.data.flags?.genemon?.uuid) {
        const item = await fromUuid(this.data.flags.genemon.uuid);
        if (item) {
          this.data = item;
        }
      }
      else if (this.data.flags?.genemon?.ffgUuid) {
        const item = await fromUuid(this.data.flags.genemon.ffgUuid);
        if (item) {
          this.data = item;
        }
      }
    ...
    if (chatData?.data?.flags?.genemon.hasOwnProperty('crew')) {
      chatData.data.crew = chatData.data.flags.genemon.crew;
    }
    if (chatData.data.hasOwnProperty('data') && (chatData.data.data.adjusteditemmodifier === undefined || chatData.data.data.adjusteditemmodifier.length === 0)) {
      ...
      if (candidate_actors.length > 0) {
        if (game.settings.get("genemon", "oldWorldCompatability")) {
        ...
        chatData.data.flags.genemon.ffgUuid = 'Actor.' + candidate_actors[0].id + '.Item.' + chatData.data._id;
      }
    }
